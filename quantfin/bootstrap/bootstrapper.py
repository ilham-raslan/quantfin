...
from quantfin.curves.interpolated_df_provider import InterpolateDFProvider
...
class MultiCurveBootstrapper:
    ...
    def bootstrap_ois(self):
        interpolated_df_provider = InterpolateDFProvider()

        for i, instrument in enumerate(self.ois_instruments):
            if isinstance(instrument, OISFuture):
                df = interpolated_df_provider.df(instrument.maturity - instrument.accrual) / (1 + instrument.accrual * instrument.market_rate())
                interpolated_df_provider.add_knot(instrument.maturity, df)
            elif isinstance(instrument, OISSwap):
                schedule = np.arange(instrument.freq, instrument.maturity + 1e-12, instrument.freq)
                k = sum(instrument.freq * instrument.fixed_rate * interpolated_df_provider.df(t) for t in schedule[:-1])
        ...
        return Curve(interpolated_df_provider)
    ...
    def bootstrap_ibor3m(self, ois_curve):
        ...
        for i, swap in enumerate(self.swaps3m):
            ...
            k = sum(ois_curve.df(time) * interpolated_df_provider.forward_rate(time - 0.25, time) * 0.25 for time in schedule[:-1])

            df = interpolated_df_provider.df(swap.maturity - 0.25) * ois_curve.df(swap.maturity) / (ois_curve.df(swap.maturity) + fixed_leg_pv - k)

            interpolated_df_provider.add_knot(swap.maturity, df)
